...
async def test_sampled_pixels(dut):
    ...
    # full frame so we are clear of the reset transient). +1: colour lags
    # hpos/vpos by one clock (see module docstring above).
    line = 100
    base = V_TOTAL * H_TOTAL + line * H_TOTAL
    for x in (0, 1, 9, 10, 99, 100, 320, 639):
        uo = await sampler.at(base + x + 1)
        assert sampler.hpos() == x + 1 and sampler.vpos() == line
        got = pixel6(uo)
